...
/**
 * A classe Arquivo le um inteiro n 
  ...
  public static void main (String[] args){
    try{
      ...
      for(int i = 0 ; i < n; i++)
      {
        ...
        arq.writeFloat(valor);
      }
    
      arq.close();
      arq = new RandomAccessFile("arquivo.txt", "r");
      // Leitura de tras pra frente
      for(int i = n - 1; i >= 0; i--)
      {
        arq.seek(i * 4); // Cada float ocupa 4 bytes
        valor = arq.readFloat();
        if(valor == (int) valor)
          MyIO.println((int) valor);
        else {
          // Remover os '0' a direita
          String valorFormatado = String.format("%.3f", valor).replaceAll("0*$", "").replaceAll("\\.$", "");
