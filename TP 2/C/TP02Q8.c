...
// Função para ordenar os shows usando Shellsort
/*
* O algoritmo Shellsort divide o array em intervalos (gaps)
* e ordena subarrays progressivamente menores,
* reduzindo os intervalos até que o array inteiro esteja ordenado.
*/
void shellsort(Show** shows, int n, int* comparacoes, int* movimentacoes) {
    // Inicializa os gaps (intervalos)
    for (int gap = n / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < n; i++) {
            Show* temp = shows[i];
            int j;

            // Realiza inserções com intervalo de "gap"
            for (j = i; j >= gap; j -= gap) {
                (*comparacoes)++; // Incrementa as comparações
                if (compararShows(shows[j - gap], temp) > 0) {
                    shows[j] = shows[j - gap];
...
int main() {
    ...
    shellsort(showsSelecionados, contador, &comparacoes, &movimentacoes);
