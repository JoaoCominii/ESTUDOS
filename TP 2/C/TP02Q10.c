...
// Função para ordenar os shows usando Quicksort
void quicksort(Show** shows, int esq, int dir, int* comparacoes, int* movimentacoes) {
    if (esq >= dir) {
        return; // Caso base: partição tem 0 ou 1 elemento
    }
    ...
    while (i <= j) {
        // Move "i" para frente enquanto o elemento for menor que o pivô
        while (compararShows(shows[i], pivo) < 0) {
            (*comparacoes)++;
            i++;
        }

        // Move "j" para trás enquanto o elemento for maior que o pivô
...
int main() {
    ...
    quicksort(showsSelecionados, 0, contador - 1, &comparacoes, &movimentacoes);
