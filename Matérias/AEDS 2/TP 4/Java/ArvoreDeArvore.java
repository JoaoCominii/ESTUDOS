...
class Show {
    ...
    public void setShow_ID(String var1) { this.Show_ID = var1; }
    public String getType() { return this.Type; }
    public void setType(String var1) { this.Type = var1; }
    public String getTitle() { return this.Title; }
    public void setTitle(String var1) { this.Title = var1; }
    public String getDirector() { return this.Director; }
    public void setDirector(String var1) { this.Director = var1; }
    public String[] getCast() { return this.Cast; }
    public void setCast(String[] var1) { this.Cast = var1; this.ordenarBubbleSort(this.Cast); }
    public String getCountry() { return this.Country; }
    public void setCountry(String var1) { this.Country = var1; }
    public String getDate_Added() { return this.Date_Added; }
    public void setDate_Added(String var1) { this.Date_Added = var1; }
    public int getRelease_Year() { return this.Release_Year; }
    public void setRelease_Year(int var1) { this.Release_Year = var1; }
    public String getRating() { return this.Rating; }
    public void setRating(String var1) { this.Rating = var1; }
    public String getDuration() { return this.Duration; }
    public void setDuration(String var1) { this.Duration = var1; }
    public String[] getListed_In() { return this.Listed_In; }
    public void setListed_In(String[] var1) { this.Listed_In = var1; this.ordenarBubbleSort(this.Listed_In); }
    ...
    public static Show[] Ler() {
        ...
        try {
            ...
            try {
                ...
                try { var3.close(); } catch (Throwable var10) { var11.addSuppressed(var10); }
                throw var11;
            }
            var3.close();
...
class NoSecundario {
    ...
    public NoSecundario(String title) {
        this.title = title;
...
public class ArvoreDeArvore {
    ...
    // Ordem fixa de criação da árvore principal
    private static final int[] ORDEM = {7, 3, 11, 1, 5, 9, 13, 0, 2, 4, 6, 8, 10, 12, 14};

    // Criação da árvore principal com ordem fixa
    private NoPrincipal criarArvorePrincipal() {
        NoPrincipal r = null;
        for (int chave : ORDEM) {
            r = inserirPrincipal(r, chave);
        }
        return r;
    }

    private NoPrincipal inserirPrincipal(NoPrincipal no, int chave) {
        if (no == null) return new NoPrincipal(chave);
        if (chave < no.chave) no.esq = inserirPrincipal(no.esq, chave);
        else if (chave > no.chave) no.dir = inserirPrincipal(no.dir, chave);
        return no;
    }

    // Busca nó principal pela chave
    private NoPrincipal buscarPrincipal(NoPrincipal no, int chave) {
        if (no == null) return null;
        if (chave == no.chave) return no;
        if (chave < no.chave) return buscarPrincipal(no.esq, chave);
        else return buscarPrincipal(no.dir, chave);
    }

    // Inserção na árvore secundária
    private NoSecundario inserirSecundario(NoSecundario no, String title) {
        if (no == null) return new NoSecundario(title);
        if (title.compareTo(no.title) < 0) no.esq = inserirSecundario(no.esq, title);
        else if (title.compareTo(no.title) > 0) no.dir = inserirSecundario(no.dir, title);
        return no;
    }
    ...
    public void inserir(Show show) {
        ...
        NoPrincipal no = buscarPrincipal(raiz, chave);
        if (no != null) {
            no.raizSecundaria = inserirSecundario(no.raizSecundaria, show.getTitle());
        }
    }

    public boolean pesquisar(String title) {
        System.out.print("raiz ");
        boolean achou = pesquisarPre(title, raiz);
        if (!achou) {
            System.out.println("NAO");
        }
        // Se achou, o próprio pesquisarPre já imprime SIM com println
        return achou;
    }

    private boolean pesquisarPre(String title, NoPrincipal no) {
        if (no == null) return false;
        if (pesquisarSecundario(title, no.raizSecundaria)) {
            System.out.println("SIM");
            return true;
        }
        System.out.print("esq ");
        if (pesquisarPre(title, no.esq)) return true;
        System.out.print("dir ");
        return pesquisarPre(title, no.dir);
    }

    private boolean pesquisarSecundario(String title, NoSecundario no) {
        if (no == null) {
            comparacoes++;
            return false;
        } else if (title.equals(no.title)) {
            comparacoes++;
            return true;
        } else if (title.compareTo(no.title) < 0) {
            comparacoes++;
            System.out.print("esq ");
            return pesquisarSecundario(title, no.esq);
        } else {
            comparacoes++;
            System.out.print("dir ");
            return pesquisarSecundario(title, no.dir);
        }
    }

    public static void main(String[] args) throws Exception {
        ArvoreDeArvore arvore = new ArvoreDeArvore();
        arvore.raiz = arvore.criarArvorePrincipal();
        ...
        int comparacoes = arvore.comparacoes;
        PrintWriter log = new PrintWriter("846713_arvoreArvore.txt");
