...
public class HashComReserva {
    private Show[] tabela;
    private int m1 = 21, m2 = 9, m = 30, reserva = 0;
    private final Show NULO = null;

    public HashComReserva() {
        tabela = new Show[m];
    ...
    public boolean pesquisar(String title) {
        int pos = hash(title);
        boolean resp = false;
        if (tabela[pos] != NULO && tabela[pos].getTitle().equals(title)) {
            resp = true;
        } else if (tabela[pos] != NULO) {
            for (int i = 0; i < reserva; i++) {
                if (tabela[m1 + i] != NULO && tabela[m1 + i].getTitle().equals(title)) {
                    resp = true;
                    break;
                }
            }
    ...
    public static void main(String[] args) throws Exception {
        HashComReserva hash = new HashComReserva();
        ...
        PrintWriter log = new PrintWriter("846713_hashReserva.txt");
