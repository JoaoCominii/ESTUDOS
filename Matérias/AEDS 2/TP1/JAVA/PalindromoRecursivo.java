...
/**
 * A classe Palindromo verifica se uma palavra é um palindromo de forma recursiva.
...
class PalindromoRecursivo 
{
  ...
  private static boolean ehPalindromo(String palavra, int esquerda, int direita) 
  {
    boolean resp;
    if (esquerda >= direita) {
      resp = true;
    } else if (palavra.charAt(esquerda) != (palavra.charAt(direita))) {
      resp = false;
    } else {
      resp = ehPalindromo(palavra, esquerda + 1, direita - 1);
    }
    return resp;
  }
