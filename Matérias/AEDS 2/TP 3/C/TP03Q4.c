...
// Insere um Show na fila circular
void inserirFila(FilaCircular *fila, Show* show) {
    if (fila->tamanho == TAM_FILA) {
        // Remove antes de inserir se estiver cheia (sem imprimir nada)
...
// Remove um Show da fila circular
...
// Calcula a média arredondada dos Release_Year dos elementos da fila
int mediaReleaseYear(FilaCircular *fila) {
    ...
    return (int)(media); // arredondado
}

// Exibe todos os elementos da fila circular no padrão pedido
void mostrarFila(FilaCircular *fila) {
    int idx = fila->primeiro;
    for (int i = 0; i < fila->tamanho; i++, idx = (idx + 1) % TAM_FILA) {
        printf("[%d] ", i);
