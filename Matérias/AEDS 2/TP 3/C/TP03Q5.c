...
// Estrutura da lista flexível (alocação dinâmica)
typedef struct {
    Show** array;
    int n;
    int capacidade;
...
// Inicializa a lista flexível
void criarLista(Lista *lista) {
    lista->capacidade = 10; // capacidade inicial
    lista->n = 0;
    lista->array = (Show**) malloc(lista->capacidade * sizeof(Show*));
}

// Garante espaço suficiente para inserir mais um elemento
void garantirEspaco(Lista *lista) {
    if (lista->n >= lista->capacidade) {
        lista->capacidade *= 2;
        lista->array = (Show**) realloc(lista->array, lista->capacidade * sizeof(Show*));
    }
}

// Insere no início
void inserirInicio(Lista *lista, Show* show) {
    garantirEspaco(lista);
...
// Libera a memória da lista flexível
...
int main() {
    ...
    // Leitura dos IDs iniciais at� encontrar "FIM"
    char id[100];
    while (scanf("%s", id) != EOF) {
        ...
        if (novoShow != NULL) {
            ...
            printf("Erro: Show ID %s n�o encontrado!\n", id);
        }
    }

    // Leitura do n�mero de opera��es
    int nOps;
    scanf("%d", &nOps);
    for (int i = 0; i < nOps; i++) {
        ...
        if (strcmp(comando, "II") == 0) {
            ...
            // Inserir em posi��o espec�fica:
            // **Ordem dos par�metros:** primeiro um inteiro (posi��o) e depois o showID.
            int pos;
            char showID[100];
            scanf("%d", &pos);
            scanf("%s", showID);
    ...
    // Exibe a lista final ap�s as opera��es
    mostrar(&lista);

    // Libera a mem�ria alocada para os Shows
