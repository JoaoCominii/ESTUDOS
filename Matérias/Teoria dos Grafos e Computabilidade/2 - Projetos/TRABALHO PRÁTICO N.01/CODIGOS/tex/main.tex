...
\title{Análise de Algoritmos de Grafos: Fleury e Detecção de Pontes}

\author{João Comini Cesar de Andrade\inst{1} }
...
\address{Instituto de Ciências Exatas e Informática -- Pontifícia Universidade Católica \\ de Minas Gerais
  ...
  \email{jccandrade@sga.pucminas.br}
}
\begin{document} 
...
Um \textbf{grafo não direcionado} $G = (V, E)$ é definido por um conjunto de vértices $V$ e um conjunto de arestas $E$, onde cada aresta conecta dois vértices distintos \cite{diestel:2017}. O \textbf{grau} de um vértice $v$, denotado $\deg(v)$, é o número de arestas incidentes a $v$. Um grafo é \textbf{conexo} se existe um caminho entre quaisquer dois vértices.
...
\begin{figure}[!tb]
\centering
\input{figures/grafo-exemplo.tikz}
\vspace{2mm}
...
Os grafos são representados internamente através de uma \textbf{lista de adjacência}: cada vértice mantém uma coleção de vizinhos. Para operações de remoção e consulta rápida de arestas durante a execução do algoritmo de Fleury, utiliza-se também um \textbf{conjunto de arestas} explícito (set), permitindo verificações de existência em tempo $O(\log m)$.
...
\begin{figure}[!tb]
\centering
\input{figures/comparacao-desempenho.tikz}
